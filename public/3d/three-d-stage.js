...
(() => {
  ...
  class ThreeDStage extends HTMLElement {
    ...
    async _boot() {
      ...
      // Soft image-based light from a sky/ground gradient: gives fabric and
      // wood real falloff instead of flat lambert shading.
      const grad = document.createElement('canvas');
      grad.width = 8; grad.height = 256;
      const gctx = grad.getContext('2d');
      const lg = gctx.createLinearGradient(0, 0, 0, 256);
      lg.addColorStop(0, '#cfe0f0');
      lg.addColorStop(0.48, '#f6f2ea');
      lg.addColorStop(0.52, '#e6ddcd');
      lg.addColorStop(1, '#9d9382');
      gctx.fillStyle = lg;
      ...
      this._key = key;
      scene.add(key);
      const fill = new THREE.DirectionalLight(0xe8eef6, 0.35);
